...
class Reducer @Inject constructor() {
    fun reduce(previousState: State, internalAction: InternalAction): State =
        when (internalAction) {
            is LoadCategories -> previousState.copy(categories = internalAction.categories)
            is SelectCategory -> previousState.copy(selectedCategoryId = internalAction.categoryId)
            is SwapCategories -> swapCategories(
                previousState,
